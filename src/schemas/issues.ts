...
export const CreateIssueSchema = z.object({
  ...
  dependencyIds: z.array(z.string()).optional().describe("dependency issue ids"),
...
export const GetProjectSchema = z.object({
  projectId: z.string().min(1, "project id is required").describe("target project id"),
});

export const GetProjectIssuesSchema = z.object({
  projectId: z.string().min(1, "target project id").describe("target project id"),
});

export const GetIssueSchema = z.object({
  issueId: z.string().min(1, "issue id is required").describe("target issue id"),
  includeComments: z.boolean().optional().describe("include comments").default(true),
  includeChildren: z.boolean().optional().describe("include children").default(true),
  includeParent: z.boolean().optional().describe("include parent").default(true),
});

export const GetViewerSchema = z.object({});

export const GetProjectStatusesSchema = z.object({}).describe("get project statuses which are linked to the project");

export const GetIssuePrioritiesSchema = z.object({}).describe("get issue priorities which are linked to the issue");
...
export const UpdateIssueStateSchema = z.object({
  issueId: z.string().min(1, "issue id is required").describe("target issue id"),
  stateId: z.enum(["todo", "in_progress", "done"]).optional().describe("issue state"),
});

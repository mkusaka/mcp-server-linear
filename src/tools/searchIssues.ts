...
export async function searchIssuesTool(args: SearchIssuesInput) {
  try {
    ...
    if (args.filter) {
      ...
      if (args.filter.description) filter["description"] = { eq: args.filter.description };
      if (args.filter.identifier) filter["identifier"] = { eq: args.filter.identifier };
      
      if (args.filter.teamId) filter["team"] = { id: { eq: args.filter.teamId } };
      if (args.filter.assigneeId) filter["assignee"] = { id: { eq: args.filter.assigneeId } };
      if (args.filter.creatorId) filter["creator"] = { id: { eq: args.filter.creatorId } };
      if (args.filter.projectId) filter["project"] = { id: { eq: args.filter.projectId } };
      if (args.filter.stateId) filter["state"] = { id: { eq: args.filter.stateId } };
      
      if (args.filter.labelIds && args.filter.labelIds.length > 0) {
        filter["labels"] = { id: { in: args.filter.labelIds } };
      }
      
      if (args.filter.priority !== undefined) filter["priority"] = { eq: args.filter.priority };
      if (args.filter.estimate !== undefined) filter["estimate"] = { eq: args.filter.estimate };
      
      if (args.filter.createdAfter) filter["createdAt"] = { gt: args.filter.createdAfter };
      if (args.filter.createdBefore) filter["createdAt"] = { lt: args.filter.createdBefore };
      if (args.filter.updatedAfter) filter["updatedAt"] = { gt: args.filter.updatedAfter };
      if (args.filter.updatedBefore) filter["updatedAt"] = { lt: args.filter.updatedBefore };
      
      if (args.filter.includeArchived !== undefined) filter["includeArchived"] = args.filter.includeArchived;
      if (args.filter.includeCanceled !== undefined) filter["includeCanceled"] = args.filter.includeCanceled;
